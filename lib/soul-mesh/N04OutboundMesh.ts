...
export interface N04OutboundOptions {
  token?: string;
  timeoutMs?: number;
}
...
export class N04OutboundMesh {
  ...
  async request<T = unknown>(
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`SOUL_MESH_REQUEST_FAILED:${peer.nucleus}:${response.status}`);
      }

      const result = await response.json() as SoulMeshMessage;
      if (result.correlationId !== correlationId) {
        throw new Error('SOUL_MESH_CORRELATION_MISMATCH');
      }
      if (result.source !== peer.nucleus || result.target !== 'N04') {
