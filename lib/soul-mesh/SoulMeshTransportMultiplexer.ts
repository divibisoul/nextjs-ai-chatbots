...
export type MeshTransportCandidate = {
  id: string;
...
/**
 * Hybrid transport fabric for N04. Transports are additive: HTTP, realtime,
 * loopback or future transports can coexist without replacing one another.
 * A send succeeds when at least one enabled transport succeeds.
 */
export class SoulMeshTransportMultiplexer implements SoulMeshTransport {
  private readonly candidates: MeshTransportCandidate[];

  constructor(candidates: MeshTransportCandidate[]) {
  ...
  async send(message: SoulMeshMessage): Promise<void> {
    ...
    for (const candidate of enabled) {
      try {
        await candidate.transport.send(message);
        return;
      } catch (error) {
  ...
  onMessage(handler: (message: SoulMeshMessage) => void | Promise<void>): () => void {
    const unsubscribers = this.candidates
      .filter((candidate) => candidate.enabled !== false)
      .map((candidate) => candidate.transport.onMessage(handler));
