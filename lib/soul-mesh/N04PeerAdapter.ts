...
export function createN04Request(target: N04Peer, capability: string, payload: unknown): SoulMeshMessage {
  return { protocol: 'soul-mesh/1', contractVersion: '1.1.0', id: randomUUID(), correlationId: randomUUID(), source: 'N04', target, kind: 'request', capability, payload, timestamp: Date.now() };
}
export async function sendFromN04(target: N04Peer, capability: string, payload: unknown, timeoutMs = 15000) {
  const peer = getN04PeerConfig().find(item => item.nucleus === target);
  if (!peer?.url) throw new Error(`SOUL_MESH_PEER_URL_NOT_CONFIGURED:${target}`);
  const message = createN04Request(target, capability, payload);
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), timeoutMs);
  try {
    const token = process.env.SOUL_MESH_TOKEN;
    const response = await fetch(`${peer.url}/api/soul-mesh`, { method: 'POST', headers: { 'content-type': 'application/json', accept: 'application/json', ...(token ? { authorization: `Bearer ${token}` } : {}) }, body: JSON.stringify(message), cache: 'no-store', signal: controller.signal });
    ...
    if (!body || body.protocol !== message.protocol || body.contractVersion !== message.contractVersion || body.correlationId !== message.correlationId || body.source !== target || body.target !== 'N04' || (body.kind !== 'response' && body.kind !== 'error')) throw new Error(`SOUL_MESH_INVALID_RESPONSE:${target}`);
