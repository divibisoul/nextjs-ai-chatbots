...
export class SoulMeshAgentRegistry {
  ...
  register(agent: Nucleus04Agent): void { this.agents.set(agent.id, agent); }
  findForCapability(capability: string): Nucleus04Agent | undefined { return [...this.agents.values()].find((agent) => agent.capabilities.includes(capability)); }
