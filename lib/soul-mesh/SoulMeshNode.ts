...
export class SoulMeshNode {
  ...
  send<T>(target: SoulNucleus, correlationId: string, payload: T, kind: SoulMeshMessage['kind'] = 'event', capability?: string): Promise<void> {
    return this.transport.send({ protocol: 'soul-mesh/1', contractVersion: '1.1.0', id: crypto.randomUUID(), correlationId, source: this.nucleus, target, kind, capability, payload, timestamp: Date.now() });
  }
}
