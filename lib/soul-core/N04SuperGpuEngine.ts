...
export interface N04SuperGpuMetrics { submitted: number; completed: number; failed: number; timedOut: number; active: number; pending: number; capacity: number; }
type Job<T> = { run: () => Promise<T>; priority: number; order: number; timeoutMs: number; resolve: (value: T) => void; reject: (error: unknown) => void };

/** Bounded software acceleration/orchestration layer; not a claim of physical GPU hardware. */
export class N04SuperGpuEngine {
  ...
  private drain() {
    while (this.active < this.capacity && this.queue.length) {
      ...
      let settled = false;
      let released = false;
      const release = () => { if (released) return; released = true; this.active = Math.max(0, this.active - 1); this.drain(); };
