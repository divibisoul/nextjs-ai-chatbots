...
export function createNucleus04MeshHandlers({ session }: Nucleus04MeshRuntimeOptions = {}) {
  ...
  const registerTool = (name: string, candidate: unknown) => {
    ...
    if (typeof execute === 'function') tools[name] = { execute: (...args) => Reflect.apply(execute, candidate, args) };
  };
  ...
  const executePilot = async (payload: unknown) => {
    ...
    const modelId = PROVIDER_MODEL_IDS.has(requestedModel) && AVAILABLE_MODELS.has(requestedModel) ? requestedModel : 'chat-model';
    return n04Cache.getOrSet(`ai:${modelId}:${stableKey({ prompt: input.prompt, system: input.system })}`, async () => {
      const result = await generateText({ model: myProvider.languageModel(modelId), system: typeof input.system === 'string' ? input.system : undefined, prompt: input.prompt });
  ...
  const handlers: Record<string, (payload: unknown) => Promise<unknown> | unknown> = {
    async 'ai-pilot'(payload) { return agents.execute({ ...message('ai-pilot', payload), capability: 'ai-pilot' }); },
    async conversation(payload) { return handlers['ai-pilot'](payload); },
    async 'tool-execution'(payload) { return agents.execute({ ...message('tool-execution', payload), capability: 'tool-execution' }); },
    async 'artifact-processing'(payload) { requireSession(session); return executeTool(payload); },
    async 'document-processing'(payload) { const input = assertObject(payload, 'DOCUMENT_PROCESSING_PAYLOAD'); return executeTool({ tool: input.operation === 'edit' || input.operation === 'update' ? 'updateDocument' : 'createDocument', arguments: input.input ?? input }); },
    async 'context-orchestration'(payload) { const input = assertObject(payload, 'CONTEXT_ORCHESTRATION_PAYLOAD'); const tasks = Array.isArray(input.tasks) ? input.tasks as Task[] : []; return { nucleus: 'N04', protocol: 'soul-mesh/1', context: input.context ?? null, results: await n04SuperGpu.map(tasks, (task) => dispatch(task.capability, task.payload), 'internal') }; },
    async 'mesh-communication'(payload) { return executeMeshCommunication(payload); },
    streaming() { throw new Error('STREAMING_REQUIRES_CHAT_TRANSPORT'); },
    async 'mesh.ping'(payload) { return agents.execute({ ...message('mesh.ping', payload), capability: 'mesh.ping' }); },
    async 'mesh.describe'() { return agents.execute({ ...message('mesh.describe', {}), capability: 'mesh.describe' }); },
    async 'core.health'() { return agents.execute({ ...message('core.health', {}), capability: 'core.health' }); },
    async 'environment.weather'(payload) { return executeTool({ tool: 'getWeather', arguments: payload }); },
    async 'batch.process'(payload) { const input = assertObject(payload, 'BATCH_PROCESS_PAYLOAD'); const tasks = Array.isArray(input.tasks) ? input.tasks as Task[] : []; return { ok: true, results: await n04SuperGpu.map(tasks, (task) => dispatch(task.capability, task.payload), 'batch') }; },
    ...
    async 'workflow.execute'(payload) { const input = assertObject(payload, 'WORKFLOW_EXECUTE_PAYLOAD'); const tasks = Array.isArray(input.tasks) ? input.tasks as Task[] : []; const results: unknown[] = []; for (const task of tasks) results.push(await dispatch(task.capability, task.payload)); return { ok: true, completed: results.length, results }; },
    async 'schedule.task'(payload) { const input = assertObject(payload, 'SCHEDULE_TASK_PAYLOAD'); const delayMs = Math.max(0, Math.min(86_400_000, Number(input.delayMs ?? 0))); if (!Number.isFinite(delayMs)) throw new Error('SCHEDULE_DELAY_INVALID'); const taskId = crypto.randomUUID(); setTimeout(() => { void dispatch(String(input.capability), input.input); }, delayMs).unref?.(); return { scheduled: true, taskId, delayMs, capability: input.capability }; },
  ...
  async function executeMeshCommunication(payload: unknown) {
