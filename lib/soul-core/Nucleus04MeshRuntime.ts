...
export function createNucleus04MeshHandlers({ session }: Nucleus04MeshRuntimeOptions = {}) {
  ...
  return {
    async 'ai-pilot'(payload: unknown) {
      ...
      const modelId: ChatModelId = isChatModelId(input.model ?? '') ? input.model! : 'chat-model';
    ...
    'mesh.ping'(payload: unknown) { return { ok: true, nucleus: 'N04', echoed: payload, processedAt: Date.now() }; },

    'mesh.describe'() {
      return {
        nucleus: 'N04', protocol: 'soul-mesh/1', capabilities: SOUL_MESH_CAPABILITIES,
        tools: ['createDocument', 'updateDocument', 'getWeather', 'requestSuggestions'], models: chatModels,
        peers: ['N01', 'N02', 'N03', 'N05', 'N06'], channels: { inbound: N04_IN_CHANNELS, outbound: N04_OUT_CHANNELS }, status: 'online',
      };
    },

    'core.health'() {
      return { ok: true, nucleus: 'N04', runtime: 'nextjs-ai-chatbots', authenticatedToolContext: Boolean(session?.user?.id), timestamp: Date.now() };
    },

    'environment.weather'(payload: unknown) {
      const weather = tools.getWeather as { execute: (args: unknown) => unknown | Promise<unknown> };
      return weather.execute(payload);
    },
  };
}
