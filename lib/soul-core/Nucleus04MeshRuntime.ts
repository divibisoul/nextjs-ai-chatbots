...
export function createNucleus04MeshHandlers({ session }: Nucleus04MeshRuntimeOptions = {}) {
  ...
  if (session) {
    tools.createDocument = { execute: async (args) => { const { createDocument } = await import('@/lib/ai/tools/create-document'); const tool = createDocument({ session, dataStream: NOOP_DATA_STREAM }); const execute = Reflect.get(tool,'execute'); if (typeof execute !== 'function') throw new Error('DOCUMENT_CREATE_HANDLER_UNAVAILABLE'); return Reflect.apply(execute, tool, [args]); } };
    tools.updateDocument = { execute: async (args) => { const { updateDocument } = await import('@/lib/ai/tools/update-document'); const tool = updateDocument({ session, dataStream: NOOP_DATA_STREAM }); const execute = Reflect.get(tool,'execute'); if (typeof execute !== 'function') throw new Error('DOCUMENT_UPDATE_HANDLER_UNAVAILABLE'); return Reflect.apply(execute, tool, [args]); } };
    tools.requestSuggestions = { execute: async (args) => { const { requestSuggestions } = await import('@/lib/ai/tools/request-suggestions'); const tool = requestSuggestions({ session, dataStream: NOOP_DATA_STREAM }); const execute = Reflect.get(tool,'execute'); if (typeof execute !== 'function') throw new Error('SUGGESTIONS_HANDLER_UNAVAILABLE'); return Reflect.apply(execute, tool, [args]); } };
  }
