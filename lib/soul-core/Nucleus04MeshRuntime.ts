...
export function createNucleus04MeshHandlers({ session }: Nucleus04MeshRuntimeOptions = {}) {
  ...
  if (session) { registerTool('createDocument', createDocument({ session, dataStream: NOOP_DATA_STREAM })); registerTool('updateDocument', updateDocument({ session, dataStream: NOOP_DATA_STREAM })); registerTool('requestSuggestions', requestSuggestions({ session, dataStream: NOOP_DATA_STREAM })); }
  ...
  const handlers: Record<string,(payload:unknown)=>Promise<unknown>|unknown> = {
    async 'ai-pilot'(payload){return agents.execute(message('ai-pilot',payload));},
    async conversation(payload){return handlers['ai-pilot'](payload);},
    async 'tool-execution'(payload){return agents.execute(message('tool-execution',payload));},
    ...
    async 'mesh.ping'(payload){return agents.execute(message('mesh.ping',payload));},
    async 'mesh.describe'(){return agents.execute(message('mesh.describe',{}));},
    async 'core.health'(){return agents.execute(message('core.health',{}));},
