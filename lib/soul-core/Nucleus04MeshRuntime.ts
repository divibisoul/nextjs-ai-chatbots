...
export function createNucleus04MeshHandlers({ session }: Nucleus04MeshRuntimeOptions = {}) {
  ...
  agents.register({
    id: 'N04-pilot-agent', name: 'N04 AI Pilot Agent', capabilities: ['ai-pilot', 'conversation'],
    execute: (m: SoulMeshMessage | { kind:'local'; nucleus:'N04'; capability:string; payload:unknown; correlationId:string }) => executePilot(m.payload),
  });
  agents.register({
    id: 'N04-tool-agent', name: 'N04 Tool Agent', capabilities: ['tool-execution', 'artifact-processing', 'document-processing', 'environment.weather'],
    ...
    execute: (m: SoulMeshMessage | { kind:'local'; nucleus:'N04'; capability:string; payload:unknown; correlationId:string }) => {
      switch (m.capability) {
        ...
        case 'mesh.describe': return {
          ...
          agents:agents.describe(), tools:['createDocument','updateDocument','getWeather','requestSuggestions'],
          models:chatModels, peers:[...PEERS], channels:{ inbound:N04_IN_CHANNELS, outbound:N04_OUT_CHANNELS }, status:'online'
        };
        default: return {
          ok:true, nucleus:'N04', runtime:'nextjs-ai-chatbots', contractVersion:SOUL_MESH_CONTRACT_VERSION,
  ...
  return {
    ...
    async 'mesh-communication'(payload:unknown){ return agents.executeLocal('mesh-communication', payload); },
    streaming(){ throw new Error('STREAMING_REQUIRES_CHAT_TRANSPORT'); },
    'mesh.handshake'(){ return agents.executeLocal('mesh.handshake', {}); },
    'mesh.ping'(payload:unknown){ return agents.executeLocal('mesh.ping', payload); },
    'mesh.describe'(){ return agents.executeLocal('mesh.describe', {}); },
    'core.health'(){ return agents.executeLocal('core.health', {}); },
    'environment.weather'(payload:unknown){ return agents.executeLocal('environment.weather', { tool:'getWeather', arguments:payload }); },
  };
}